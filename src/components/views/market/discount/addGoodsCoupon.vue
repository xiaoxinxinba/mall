    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="demo-ruleForm">
      ...
      <el-form-item label="满减金额" label-width="100px">
        <div class="add_prepend">
          ...
          <el-input style="width: 200px; margin-left: -5px;" v-model="ruleForm.mj" maxlength="8" placeholder="请输入金额">
...
<script>
  ...
  import InsertCouponActivity from '../../../../api/ShopActivity/InsertCouponActivity.js'
  import GetCouponAcivity from '../../../../api/ShopActivity/GetCouponAcivity.js'
  export default {
    mounted () {
    ...
    methods: {
      ...
      addDiscountPost(){
        ...
        console.log(this.ruleForm.day+'  111111111111')
        ...
        for(let i = 0;i<this.goodsList.length;i++){
          goodsList.push({article_id:this.goodsList[i].articleId})
        }
        let rules = []
        for (let i = 0;i<this.tags.length;i++) {
          rules.push({meet_price:this.tags[i].meet_price,discount:this.tags[i].discount,receive_meetprice:0,validays:0,limit_number:this.tags[i].limit_number,receive_number:0,user_number:0})
        }
        ...
        let data = {
          ...
          "coupon": {
            ...
            "begin_time": date1+' 00:00:00',
            "end_time": date2+' 23:59:59',
        ...
        InsertCouponActivity.call(this,data).then(res=>{
          if(res.data.code === 1){
            this.$message({
              message: '操作成功',
              type: 'success'
      ...
      add(){
        ...
        if(parseFloat(this.ruleForm.gwm) <= 0){
          this.$message({
            message: '满减金额必须是大于0!',
            type: 'warning'
          })
        ...
        if(parseFloat(this.ruleForm.gwm) > 100){
          this.$message({
            message: '满金额不得高于100元!',
            type: 'warning'
          })
        ...
        if(regPos.test(this.ruleForm.mj) || regNeg.test(this.ruleForm.mj)){
        }else{
          this.$message({
            message: '减金额必须是数字!',
            type: 'warning'
        ...
        if(parseFloat(this.ruleForm.mj) <= 0){
          this.$message({
            message: '减金额必须大于0!',
            type: 'warning'
          })
        ...
        if(parseFloat(this.ruleForm.mj) < 10){
          this.$message({
            message: '减金额为10元及以上5的整倍数!',
            type: 'warning'
          })
          return
        }
        if(parseFloat(this.ruleForm.mj) >= parseFloat(this.ruleForm.gwm)){
          this.$message({
            message: '消费金额必须大于减金额!',
            type: 'warning'
          })
        ...
        this.tags.push({id:`${parseFloat(this.ruleForm.gwm)}${parseFloat(this.ruleForm.mj)}`,
          name: `满${parseFloat(this.ruleForm.gwm)}减${parseFloat(this.ruleForm.mj)}元 | ${this.ruleForm.ffsl}张`
          ,meet_price:parseFloat(this.ruleForm.gwm)
          ,discount:parseFloat(this.ruleForm.mj)
      ...
      changeGoods(val){
        if(val === 'part'){
          ...
          //this.totalCount = this.allTotalCount
          this.isAllProduct = 1
        }
      },
    ...
    created:async function(){
      ...
      if(type === 'edit' && activityId !== '' && activityId !== undefined){
        this.time = new Date().toLocaleString()
        GetCouponAcivity.call(this,`?input.activityId=${activityId}&input.clientVersion=v1&input.clientTimespan=${this.time}`).then(res=>{
          if(res.data.code === 1){
            var data = res.data.data
            if(data !== null){
              ...
              for(let i = 0;i< data.rules.length;i++){
                ...
                this.tags.push({id:`${parseFloat(data.rules[i].meet_price)}${parseFloat(data.rules[i].discount)}`,
                  name: `满${parseFloat(data.rules[i].meet_price)}减${parseFloat(data.rules[i].discount)}元 | ${data.rules[i].give_number}张`
