  ...
  <div class="addDiscount_box">
    ...
    <el-form class="demo-ruleForm">
      ...
      <el-form-item label="满减金额" label-width="100px">
        ...
        <div class="add_tag">
          <el-tag v-for="(tag, i) in tags" :key="i" :type="tag.type">满{{tag.meet_price}}减{{tag.discount}}元| {{tag.give_number}}张</el-tag>
        </div>
      </el-form-item>
          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <span class="look_span">满{{scope.row.meet_price}}减{{scope.row.discount}}元| {{scope.row.limit_number}}张</span>
              </template>
            </el-table-column>
        ...
        <div class="table table1">
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <div class="shopName">
                  <p>{{scope.row.sell_price | moneyFormat}}</p>
                </div>
              </template>
            </el-table-column>
          </el-table>
...
<script>
  import GetCouponAcivity from '../../../../../src/api/ShopActivity/GetCouponAcivity.js'
  export default {
    mounted () {
    ...
    methods: {
      ...
      GetCouponAcivity(activityId){
        ...
        GetCouponAcivity.call(this,`?input.activityId=${activityId}&input.clientVersion=&input.clientTimespan=${this.time}`)
          .then(res=>{
            if(res.data.code === 1){
